...
//-----------------------------------------------------------------------------
	...
	IsInitialized: function() {
		...
		try {
			isInitialized = 
				(Module._EUIsInitialized() == EU_TRUE);
		} catch (e) {
			isInitialized = false;
		}
...
//-----------------------------------------------------------------------------
	DoesNeedSetSettings: function() {
		try {
			return (Module._EUDoesNeedSetSettings() == EU_TRUE);
		} catch (e) {
...
//-----------------------------------------------------------------------------
	IsPrivateKeyReaded: function() {
		...
		try {
			isReaded = (Module._EUIsPrivateKeyReaded() == EU_TRUE);
			error = EU_ERROR_NONE;
		} catch (e) {
			error = EU_ERROR_UNKNOWN;
			isReaded = false;
	...
	GetPrivateKeyOwnerInfo: function() {
		var error = EU_ERROR_NONE;
		try {
			if (Module._EUIsPrivateKeyReaded() != EU_TRUE) {
				this.privKeyOwnerInfo = null;
				error = EU_ERROR_UNKNOWN;
			}
		} catch (e) {
