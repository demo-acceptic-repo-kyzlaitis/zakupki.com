...
use App\Api\Api;
use App\Api\Struct\PlanDocument;
...
class PlanDocumentUploadListener implements ShouldQueue
{
    use DispatchesJobs;
    /**
     ...
     * @return void
     */
    public function handle(PlanDocumentUploadEvent $event)
    {
        $document = $event->document;
        $api = new Api();
        $documentStruct = new PlanDocument($document);
        ...
        if ($api->responseCode == 201) {
            ...
            $this->dispatch((new UploadDocument(new PlanDocument($document)))->onQueue('uploads'));
